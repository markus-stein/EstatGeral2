...
* **(cont.) Exemplo**: Suponha que $\overline x = `r round(mean(x),3)`$ para uma amostra observada, nesse caso rejeitamos $H_0$, porque $\overline x = `r xbarra` > `r xtab` = \overline x_{crítico}$; ou $z_{calc} = `r zcalc` > `r ztab` = z_{crítico}$.
    + E qual o valor *p*?
...
ggplot(data = gg, mapping = aes(x = X, y = YX)) +
  ...
  annotate("text", x = m - 3*sxbarra,
    ...
    annotate("text", x = m + 3*sxbarra,
           ...
           label = paste("valor *p* = ", round( pnorm(xbarra, m, sxbarra, lower.tail = FALSE),4))) +
  scale_x_continuous(breaks = seq(m - 3*sxbarra, m + 3*sxbarra, sxbarra*1.5)) +
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))
...
ggplot(data = gg, mapping = aes(x = Z, y = YZ)) +
  ...
  annotate("text", x = -3, y = max(YZ)/2,    #Y[which(grepl(m - s, X))],
    ...
    annotate("text", x = 3, y = max(YZ)/4,    #Y[which(grepl(m - s, X))],
           label = paste("valor *p* = ", round( pnorm(zcalc, lower.tail = FALSE),4))) +
  scale_x_continuous(breaks = seq(-3, 3, 1.5)) +
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))
```
